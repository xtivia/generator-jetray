import {UIRouter,StateDeclaration} from "ui-router-ng2";
...
export function uiRouterConfigFn(router: UIRouter, injector: Injector) {
  ...
  router.urlService.rules.otherwise(function(_router,_url) {
    //console.log("Angular router handling unknown url="+ _url.path);
    let currentState = router.stateService.current.name;
    if (!currentState) {
      router.stateService.go('home',null,{location:false});
      //console.log("Angular router resetting to home state");
    }
    return;
  });
